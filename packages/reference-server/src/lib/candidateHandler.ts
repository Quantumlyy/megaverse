import type { ComethDirection, Grid, ServerCell, SoloonColor } from "@megaverse/core";
import { constants, utils } from "@megaverse/core";
...
export function placePolyanet(candidateId: string, row: number, column: number) {
  ...
  // biome-ignore lint/style/noNonNullAssertion: We initialized the grid if it was missing
  grid[row]![column] = { type: 0 };
}

export function placeSoloon(candidateId: string, row: number, column: number, color: SoloonColor) {
  const { grid } = getOrCreate(candidateId);
  // biome-ignore lint/style/noNonNullAssertion: We initialized the grid if it was missing
